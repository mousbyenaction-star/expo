...
internal class PerfMonitorDataSource() : DevToolsReactPerfLogger.DevToolsReactPerfLoggerListener {
  ...
  fun start(reactContext: ReactApplicationContext) {
    UiThreadUtil.runOnUiThread {
      if (fpsRunnable != null) return@runOnUiThread
  ...
  private fun attachFabricListenerIfNeeded(reactContext: ReactApplicationContext) {
  ...
  private class FpsBuffer(private val label: String, private val trackType: TrackType) {
  ...
  private class ChoreographerFpsTracker(
    ...
    fun getFpsAndReset(): Double {
      ...
      val fps = if (elapsedSeconds > 0) frameCount / elapsedSeconds else 0.0
